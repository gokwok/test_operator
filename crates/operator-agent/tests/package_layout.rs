...
#[test]
fn operator_agent_package_exports_library_only() {
    ...
    let package = metadata["packages"]
        ...
        .and_then(|packages| {
            packages
                .iter()
                .find(|package| package["name"].as_str() == Some("operator-agent"))
        })
        .expect("operator-agent package should exist");
    ...
    assert!(
        ...
        "operator-agent should keep exporting a library target"
    );
    assert!(
        ...
        "operator-agent should not expose a binary target"
    );
}
